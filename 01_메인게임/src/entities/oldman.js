...
import oldmanLines from "../content/temp/oldmanDialogue.js";
...
export async function startInteraction(k, oldman, player) {
    ...
    const font = gameState.getLocale() === "korean" ? "galmuri" : "gameboy";

    playerState.setIsSwordEquipped(true);

    if (gameState.getIsSonSaved()) {
        await dialog(k, k.vec2(250, 500), responses[3], { font });
        return;
    }
    ...
    if (responses[nbTalkedOldMan]) {
        await dialog(k, k.vec2(250, 500), responses[nbTalkedOldMan], { font });
