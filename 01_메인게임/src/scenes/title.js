...
import { colorizeBackground, audioManager } from "../utils.js"; // 비타일맵 씬이지만 fadeInBGM 사용하지 않음
...
export default function title(k) {
    ...
    // 페이드 인 애니메이션
    k.tween(fadeIn.opacity, 0, 2.0, (val) => {
        fadeIn.opacity = val;
    }).then(() => {
        fadeIn.destroy();
    });
    ...
    if (!audioManager.isBGMPlaying() || audioManager.getCurrentBGM() !== "title-bgm") {
        ...
        const bgmResult = audioManager.switchBGM("title-bgm", 1.0);
        if (bgmResult) {
    ...
    // HTML Canvas 오버레이 생성 (화면 전체를 덮도록)
    let canvas = document.createElement('canvas');
    let ctx = null; // ctx를 전역 변수로 명시적 선언
    ...
    const scale = Math.min(scaleX, scaleY); // 비율 유지하면서 화면에 맞춤
    ...
    canvas.style.zIndex = '9999'; // z-index를 더 높게 설정
    canvas.style.pointerEvents = 'none';
    canvas.style.backgroundColor = 'black'; // 검은색 배경으로 변경
    ...
    // 저작권 표시 (애니메이션 초반부터 표시)
    const copyrightText = k.add([
        k.text("ⓒ 2025. Team (공)동체 All rights reserved.", {
            size: 14,
            font: "galmuri",
            align: "center"
        }),
        k.pos(k.width() / 2, 30), // 화면 상단에서 30px 아래
        k.anchor("center"),
        k.color(255, 255, 255), // 흰색 텍스트
        k.opacity(1.0), // 완전 불투명
        k.z(1000), // 매우 높은 z-index로 모든 요소 위에 표시
        k.fixed(), // 화면에 고정
    ]);

    console.log("저작권 텍스트 생성됨:", copyrightText.text, "위치:", copyrightText.pos);
    ...
    k.onSceneLeave(() => {
        try {
            ...
            console.log("✅ Title 씬 종료 시 문서 배경 복원 완료");
        } catch (e) {
            console.log("⚠️ Canvas/배경 정리 중 오류:", e.message);
        }
    });
}
