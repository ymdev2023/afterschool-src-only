...
export default function prologue_short(k) {
    console.log("🎭 Prologue Short 씬 시작");
    ...
    // 페이드 인 완료 후 오버레이 제거
    setTimeout(() => {
        console.log("✨ Prologue Short Fade In 완료");
    ...
    // 짧은 프롤로그 대화 내용 (한 번에 풀네임 입력)
    const prologueLines = [
        { text: "...뭐지...? 분명 뉴콘텐츠 쇼케이스 전시 중이었는데...", type: "thought", speaker: "플레이어" },
        { text: "거기 학생!!! 학생 이름이 뭐라고 했지?", type: "dialogue", speaker: "???" },
        { text: "이름을 알려주자!", type: "question", speaker: "플레이어" },
        { text: "저는...full_name인데요...", type: "dialogue", speaker: "full_name" },
        { text: "(...내가...은하여고에 전학을 가게 된다고...?!)", type: "thought", speaker: "full_name" }
    ];
    ...
    function typeText(line) {
        ...
        // 이름 입력 라인(2)인지 확인
        const isNameInputLine = currentLineIndex === 2;
        ...
        // 플레이어 이름 치환
        if (displayText.includes("full_name")) {
            // full_name - 풀네임
    ...
    function startNameInput() {
        ...
        // 프롬프트 텍스트 설정
        inputPrompt.text = "이름을 입력하고 ENTER를 누르세요 (한글/영어 2자 이상)";
    ...
    function handleNameSubmit() {
        // 이름 입력 처리 (2자 이상, 한글/영어만)
        if (playerName.trim().length >= 2 && isValidFinalName(playerName)) {
            // 이름 입력 완료
            ...
            // 이름이 2자 미만이거나 유효하지 않을 때 경고 메시지
            if (playerName.trim().length < 2) {
                console.log("이름은 2자 이상 입력해야 합니다.");
            } else {
    ...
    function proceedToNext() {
        ...
        if (currentLineIndex >= prologueLines.length - 1) {
            console.log("프롤로그 쇼트 마지막 라인에 도달함 - 프롤로그 종료 처리 시작");
            ...
            // 약간의 지연 후 종료 처리 시작
            k.wait(1, () => {
                console.log("🎵 프롤로그 쇼트 종료 - BGM 볼륨 페이드 아웃과 화면 페이드아웃 동시 시작");
                ...
                console.log("🌙 Prologue Short Fade Out 시작");
                ...
                k.tween(fadeOut.opacity, 1, 1.5, (val) => {
                    ...
                    console.log("✨ Prologue Short 완료 - Tutorial 씬으로 전환");
        ...
        // 특별한 조건: 이름 입력이 필요한 라인(2번)에서는 이름이 입력되지 않으면 진행 불가
        if (currentLineIndex === 2 && (!playerName || playerName.trim().length === 0)) {
    ...
    k.onSceneLeave(() => {
        ...
        console.log("✅ Prologue Short 씬 종료");
    });
}
